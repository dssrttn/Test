...
DIL_1_rel<- read_csv2("data/DIL_1.csv") # read csv mit europaeischer Syntax. Auf das richtige package achten! 

DIL_1_rel <- distinct(DIL_1_rel, Mittelwert) # aus dem ursprünglichen tibble wird nur die column Mittelwert behalten
  
DIL_1_rel <- ddply(DIL_1_rel, .(), transform, abschange = (Mittelwert[1] - Mittelwert)) # absolute Differenzen zum Ausgangswert in der 1. Reihe 
...
DIL_1_rel <- ddply(DIL_1_rel, .(), transform, relchange = ((Mittelwert - Mittelwert[1]) / Mittelwert[1]) * 100) # relative Differenzen zum Ausgangswert in der 1. Reihe 

add_column(DIL_1_rel, x= 1:14)
...
x <- (1:14)
y <- DIL_1_rel$relchange

ggplot(DIL_1_rel,aes(x=x #DIL_relabs$x
 ...
 expand_limits(y = c(-2, 2)) +
  scale_y_continuous(breaks = seq(-2, 2, 1.0)) +
...
geom_abline(intercept = 76.23, slope = 0, lty=1) + # horizontale Linie ohne Steigung, da slope = 0 
