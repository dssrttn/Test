...
ggplot(DIL_mean_rel,aes(x=1:nrow(DIL_mean_rel) 
                      ,y=y )) + 
   geom_segment( aes(x=1:nrow(DIL_mean_rel), xend=1:nrow(DIL_mean_rel), y=0, yend=y), color="black") + # Striche von der Hoehe y bis zu den Punkten
    geom_point() +
scale_y_continuous(expand=c(0.1, .01), breaks = pretty_breaks(), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when one does: scale_y_continuous(labels = scales::percent)
#   scale_x_continuous( breaks = pretty_breaks())  + # adds ticks automatically
...
ggplot(Boost_mean_rel,aes(x=1:nrow(Boost_mean_rel)
  ...
  geom_point() +
   scale_y_continuous(breaks = pretty_breaks(), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when on e does: scale_y_continuous(labels = scales::percent)
 ...
 geom_point(data = DIL_mean_rel_table, aes(x = 1:nrow(DIL_mean_rel_table), y = DIL_mean_rel$relchange, color = "DIL")) +
  ...
  scale_y_continuous(breaks = seq(-2, 2, 1.0), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when one does: scale_y_continuous(labels = scales::percent)
    scale_x_continuous(breaks = c(1,5,10,nrow(DIL_mean_rel_table)))  + # adds only specific ticks
      geom_abline(intercept = 0, slope = 0, lty=1, col='black') + # horizontale Linie ohne Steigung, da slope = 0 
  ggtitle("DIL - Relative dose changes in percent from the 1st fraction with DIL") +
  xlab("Fraction") + # X-Achsen-Beschriftung
  ylab("Relative dose changes") +   #+ # Y-Achsen-Beschriftung
  plottheme
...
ggplot(Rectum_mean_rel, aes(x=1:nrow(Rectum_mean_rel)
  ...
  geom_point() +
    scale_y_continuous(breaks = pretty_breaks(), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when one does: scale_y_continuous(labels = scales::percent)
...
# expand_limits(y = c(-5, 10)) +
  ...
  #pretty_breaks()
   ...
   # scale_y_continuous(breaks = c(min(y), max(y)), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when one does: scale_y_continuous(labels = scales::percent)
  #scale_y_continuous(breaks = seq(-5, 10, 1.0), labels=function(x) paste0(x,"%")) + #labels=function(x) paste0(x,"%") adds percent symbol, but without multiplying it with 100, like when one does: scale_y_continuous(labels = scales::percent)
