...
## 1. Versuch Tufte-like in r base
```{r echo = FALSE, fig.width=11, fig.height=7, dev=c('png', 'pdf')}
par(family="serif") # Schriftart
par(mar = c(6, 8, 2, 2)) # Abstände um den Plot. 2. Po´ition y-Achse
par(mgp=c(4,1,0)) # mpg: Abstand Label zu Achse
plot(x, y, ylim=c(74, 80.2), xlab="Fraktion", ylab="Dosis in Gray", family="serif", cex = 1 # Größe der Punkte im plot
     , axes = FALSE #ticks=TRUE
     , pch=16, #main="Dosisverteilung DIL Mittelwert 1"
     type="b")
text(x=13.0, y = 75.8, labels="Dosisvorgabe Boost", family="serif", cex = 1.4) 
text(x=11.9, y = 80.2, labels="+5% Abweichung von Dosisvorgabe", family="serif", cex = 1.4)
axis(1, at=x, label=x, tick=F, cex.lab=5, cex.axis=2, family="serif")
